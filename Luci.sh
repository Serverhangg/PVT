...
cat <<EOL > .devcontainer/devcontainer.json
{
    ...
    "postStartCommand": "echo '*'; echo '*         @Luci_Magic Bot Running      *'; echo '*'; python /workspaces/PVT/$(basename "$PYTHON_FILE")",
    "postCreateCommand": "pip install pymongo python-telegram-bot pyTelegramBotAPI certifi && chmod +x /workspaces/PVT/*",
    "customizations": {
        "vscode": {
            "settings": {
